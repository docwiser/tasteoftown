...
const filteredUsers = computed(() => {
  ...
  return users.value.filter(user => {
    // Search by UID, Name, Email, Profile Phone
    if (user.uid?.toLowerCase().includes(query)) return true;
    if (user.displayName?.toLowerCase().includes(query)) return true;
    if (user.email?.toLowerCase().includes(query)) return true;
    if (user.phone?.toLowerCase().includes(query)) return true;

    // Search by address phone numbers and pincodes
    if (user.addresses && Array.isArray(user.addresses)) {
      for (const address of user.addresses) {
        if (address.phone?.toLowerCase().includes(query)) return true;
        if (address.pincode?.toLowerCase().includes(query)) return true;
      }
    }
      ...
      <div class="section">
        <div class="section-header">
          ...
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Search by UID, name, email, phone, pincode..."
            class="search-input"
          />
        </div>
              ...
              <tr v-for="user in filteredUsers" :key="user.id">
                ...
                <td>
                  ...
                  <div v-if="user.addresses && user.addresses.length">
                    <div v-for="(address, i) in user.addresses" :key="i" class="address-phone">
                      {{ address.phone }} ({{ address.label }})
                    </div>
                  </div>
                </td>
                <td>
