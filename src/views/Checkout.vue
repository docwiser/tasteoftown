...
const applyCoupon = async (coupon) => {
  if (coupon.userId && coupon.userId !== user.value.uid && coupon.userId !== user.value.email) {
    error.value = 'This coupon is not valid for your account.';
    return;
  }

  if (cartTotal.value < coupon.minOrder) {
    error.value = `Minimum order of ₹${coupon.minOrder} is required to use this coupon.`;
    return;
  }

  if (coupon.oneTimeUse) {
    if (!selectedAddress.value || !selectedAddress.value.phone) {
      error.value = 'Please select a delivery address with a phone number to use this coupon.';
      return;
    }
    const { data: orders, error: ordersError } = await getDocuments('orders', [
      { type: 'where', field: 'coupon', operator: '==', value: coupon.code },
      { type: 'where', field: 'address.phone', operator: '==', value: selectedAddress.value.phone }
    ]);

    if (ordersError) {
      error.value = 'Error checking coupon usage. Please try again.';
      return;
    }

    if (orders && orders.length > 0) {
      error.value = 'You have already used this one-time coupon.';
      return;
    }
  }
  ...
  if (coupon.type === 'percentage') {
    calculatedDiscount = (cartTotal.value * coupon.value) / 100;
...
const placeOrder = async () => {
  ...
  const orderData = {
    ...
    coupon: appliedCoupon.value ? { code: appliedCoupon.value.code, oneTimeUse: appliedCoupon.value.oneTimeUse || false } : null,
          ...
          <div class="order-summary">
            ...
            <div class="coupon-section">
              <div class="section-header-inline">
                <h3>Apply Coupon</h3>
                <button @click="showCouponsModal = true" class="btn-view-coupons">View Coupons</button>
              </div>
